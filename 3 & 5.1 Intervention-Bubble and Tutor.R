...
###################################################################################################################
######### input #########
myfun3 <- function(# N.Staff, # total staffs at school
  ...
  for (i in 1:T){
    for (j in 1:bubble.no){  
      ...
      r3[i+1,j] =  S[i,j]
       ...
       ######### TUTOR GROUP EFFECT ON STUDENT #########
      
       ######### STUDENTs TO STUDENTs in the same tutorial group #########
        # A tutor group contains 20 students and a staff that is not teaching the bubbles
        ...
        # S.Std1 : the change of student in bubble j by infections from students in the tutor group
        # S.Std2 : the change of studnets in bubble j by infections from the tutor
        # initial - there is no change of S and E initially
      
      
        S.Std_Std[1,] = c(rep(0,B.Student))  # S.Std_Std : total change of students in tutor group
        S.Std1[1,] = c(rep(0,B.Student))
        S.Std2[1,] = c(rep(0,B.Student)) 
        
      
        # The change of S for tutor group j with students in both bubble j and 11-j
        if (j <= bubble.no * B.Student / 20){ # tutor groups in total
          if (B.Student == 10){
            S.Std_Std[i+1,j] = - (S[i,j]+S[i,bubble.no+1-j])*p[1]*((D[i,j]+e*U[i,j])+(D[i,bubble.no+1-j]+e*U[i,bubble.no+1-j]))*1/24
            
            if (S[i,j]+S[i,bubble.no+1-j] > 0){
              S.Std1[i+1,j] = S.Std_Std[i+1,j]*S[i,j]/(S[i,j]+S[i,bubble.no+1-j])  # S.Std1 : total change of students in bubble j by having tutor group
              S.Std1[i+1,bubble.no+1-j] = S.Std_Std[i+1,j]*S[i,bubble.no+1-j]/(S[i,j]+S[i,bubble.no+1-j])
              
              S.Std2[i+1,j] = - ((S[i,j]+S[i,bubble.no+1-j])*p[3]*(D.Stf[i,j]+e*U.Stf[i,j])*1/24)*(S[i,j]/(S[i,j]+S[i,bubble.no+1-j]))
              S.Std2[i+1,bubble.no+1-j] = - ((S[i,j]+S[i,bubble.no+1-j])*p[3]*(D.Stf[i,j]+e*U.Stf[i,j])*1/24)*(S[i,bubble.no+1-j]/(S[i,j]+S[i,bubble.no+1-j]))
              }
            
            else{
              S.Std1[i+1,j] = 0
              S.Std1[i+1,bubble.no+1-j] = 0
              
              S.Std2[i+1,j] = 0
              S.Std2[i+1,bubble.no+1-j] = 0
            }
            }
          
          
          else if (B.Student == 5){
            S.Std_Std[i+1,j] = - (S[i,j]+S[i,bubble.no/2+1-j]+S[i,bubble.no/2+j]+S[i,bubble.no+1-j])*p[1]*((D[i,j]+e*U[i,j])+(D[i,bubble.no/2+1-j]+e*U[i,bubble.no/2+1-j])+(D[i,bubble.no/2+j]+e*U[i,bubble.no/2+j])+(D[i,bubble.no+1-j]+e*U[i,bubble.no+1-j]))*1/24
            
            if (S[i,j]+S[i,bubble.no/2+1-j]+S[i,bubble.no/2+j]+S[i,bubble.no+1-j] > 0){
              S.Std1[i+1,j] = S.Std_Std[i+1,j]*S[i,j]/(S[i,j] + S[i,bubble.no/2+j] + S[i,bubble.no/2+1-j] + S[i,bubble.no+1-j])  # S.Std1 : total change of students in  bubble j by having tutor group
              S.Std1[i+1,bubble.no/2+1-j] = S.Std_Std[i+1,j]*S[i,bubble.no/2+1-j]/(S[i,j] + S[i,bubble.no/2+j] + S[i,bubble.no/2+1-j] + S[i,bubble.no+1-j])
              S.Std1[i+1,bubble.no/2+j] = S.Std_Std[i+1,j]*S[i,bubble.no/2+j]/(S[i,j] + S[i,bubble.no/2+j] + S[i,bubble.no/2+1-j] + S[i,bubble.no+1-j])
              S.Std1[i+1,bubble.no+1-j] = S.Std_Std[i+1,j]*S[i,bubble.no+1-j]/(S[i,j] + S[i,bubble.no/2+j] + S[i,bubble.no/2+1-j] + S[i,bubble.no+1-j])
              
              S.Std2[i+1,j] = - ((S[i,j]+S[i,bubble.no/2+1-j]+S[i,bubble.no/2+j]+S[i,bubble.no+1-j])*p[3]*(D.Stf[i,j]+e*U.Stf[i,j])*1/24)*(S[i,j]/(S[i,j]+S[i,bubble.no/2+1-j]+S[i,bubble.no/2+j]+S[i,bubble.no+1-j]))
              S.Std2[i+1,bubble.no/2+1-j] = - ((S[i,j]+S[i,bubble.no/2+1-j]+S[i,bubble.no/2+j]+S[i,bubble.no+1-j])*p[3]*(D.Stf[i,j]+e*U.Stf[i,j])*1/24)*(S[i,bubble.no/2+1-j]/(S[i,j]+S[i,bubble.no/2+1-j]+S[i,bubble.no/2+j]+S[i,bubble.no+1-j]))
              S.Std2[i+1,bubble.no/2+j] = - ((S[i,j]+S[i,bubble.no/2+1-j]+S[i,bubble.no/2+j]+S[i,bubble.no+1-j])*p[3]*(D.Stf[i,j]+e*U.Stf[i,j])*1/24)*(S[i,bubble.no/2+j]/(S[i,j]+S[i,bubble.no/2+1-j]+S[i,bubble.no/2+j]+S[i,bubble.no+1-j]))
              S.Std2[i+1,bubble.no+1-j] = - ((S[i,j]+S[i,bubble.no/2+1-j]+S[i,bubble.no/2+j]+S[i,bubble.no+1-j])*p[3]*(D.Stf[i,j]+e*U.Stf[i,j])*1/24)*(S[i,bubble.no+1-j]/(S[i,j]+S[i,bubble.no/2+1-j]+S[i,bubble.no/2+j]+S[i,bubble.no+1-j]))
              
              }
            
            else{
              S.Std1[i+1,j] = 0
              S.Std1[i+1,bubble.no/2+1-j] = 0
              S.Std1[i+1,bubble.no/2+j] = 0
              S.Std1[i+1,bubble.no+1-j] = 0
              
              S.Std2[i+1,j] = 0
              S.Std2[i+1,bubble.no/2+1-j] = 0
              S.Std2[i+1,bubble.no/2+j] = 0
              S.Std2[i+1,bubble.no+1-j] = 0
              }
            }
          
          else { # B.Student == 20
            S.Std_Std[i+1,j] = - S[i,j]*p[1]*(D[i,j]+e*U[i,j])*1/24
            S.Std1[i+1,j] = S.Std_Std[i+1,j] # S.Std1 : total change of students in bubble j by having tutor group
            
            S.Std2[i+1,j] = - S[i,j]*p[3]*(D.Stf[i,j]+e*U.Stf[i,j])*1/24
        ...
        # initial - there is no change of S and E initially
        S.Stf[1,] = c(rep(1,bubble.no*B.Student/20), rep(0,bubble.no - bubble.no*B.Student/20)) 
        E.Stf[1,] = D.Stf[1,] = U.Stf[1,] = Q.Stf[1,] = R.Stf[1,] = rep(0,bubble.no)
        
        # The total number of S for staff in tutor group j 
        if (j <= bubble.no*B.Student/20){ # 5 tutor groups in total
          
          if (S.Stf[i,j] > 0){
            
            if (B.Student == 10) {
              S.Stf[i+1,j] = S.Stf[i,j] - S.Stf[i,j]*p[4]*((D[i,j]+e*U[i,j])+(D[i,bubble.no/2+1-j]+e*U[i,bubble.no/2+1-j]))*1/24
            }
            
            else if (B.Student == 5) {            
              S.Stf[i+1,j] = S.Stf[i,j] - S.Stf[i,j]*p[4]*((D[i,j]+e*U[i,j])+(D[i,bubble.no/2+1-j]+e*U[i,bubble.no/2+1-j])+(D[i,bubble.no/2+j]+e*U[i,bubble.no/2+j])+(D[i,bubble.no+1-j]+e*U[i,bubble.no+1-j]))*1/24
            }
            
            else {
              S.Stf[i+1,j] = S.Stf[i,j] - S.Stf[i,j]*p[4]*(D[i,j]+e*U[i,j])*1/24
            }
          }
...
###################################################################################################################
# plots <- myfun3(B.Staff = 3, B.Student = 10, bubble.no = 10, 
...
# This is the same as Variation in Time: Monday-Friday 
myfun5.1 <-myfun3 # Normal weekend: Monday-Friday
